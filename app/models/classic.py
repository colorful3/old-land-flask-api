# -*- coding: utf8 -*-
from flask import g
from sqlalchemy import Column, Integer, Text, String, SmallInteger, ForeignKey
...
class Classic(Base):
    ...
    def favor_info(type, id):
        uid = g.user.uid
        res = Like.query.filter_by(
            cid=id, uid=uid, type=type
        ).count()
        fav_nums = Like.query.filter_by(
            cid=id, type=type
        ).count()
        result = dict(
            fav_nums=fav_nums,
    ...
    def my_favor(start=1, count=20):
        ...
        like_list = Like.query.filter(
            *filters
        ).paginate(int(start), int(count), False)
        items = like_list.items
        res = [Classic.query.filter_by(
            id=item.cid
        ).first() for item in items]
        return res
    ...
    def like_status(self):
        ...
        if not uid:
            ...
            status = 1 if Like.query.filter_by(
                cid=self.id, uid=uid
            ).first() else 0
        return status
